...
//        combatLoop();
    ...
    private List<Pair<Room, Integer>> calculateDamage(List<Pair<Room, Weapon>> turnReport, Ship shipFiring,
        ...
        for (Pair<Room, Weapon> shot : turnReport) {
            ...
            if (pickRandom() > (weapon.getBaseChanceToHit() * shipFiring.getBaseAccuracy())) {
                damage = 0;
            } else if (pickRandom() > shipFiredAt.calculateEvadeChance()) {
                damage = 0;
            } else {
                damage = weapon.getBaseDamage();
                if (pickRandom() > (weapon.getBaseCritChance() * shipFiring.getBaseCritChance)) {
                    damage *= 1.5;
                }
            }
     ...
     *
     * @param attackReport A report generated by CombatActor which tells the game which weapons were fired and at
     *                     which
